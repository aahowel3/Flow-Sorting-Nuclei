#USAGE: command line input should look like $bash flowsort_curation_3.sh *_sorted_rmdup.bam
...
#strips extension from input file 
	file=$(basename "$filename" _sorted_rmdup.bam)
...
#	samtools view -f 0x0080 "${file}"_sorted_rmdup.bam | cut -f1,2,5 > MQ_txtfiles/"${file}"_MQ_second.txt 
	samtools view -f 0x0040 -F 3840 "${file}"_sorted_rmdup.bam | cut -f1,2,5 > MQ_txtfiles/"${file}"_MQ_first_pri.txt
      	samtools view -f 0x0080 -F 3840 "${file}"_sorted_rmdup.bam | cut -f1,2,5 > MQ_txtfiles/"${file}"_MQ_second_pri.txt
